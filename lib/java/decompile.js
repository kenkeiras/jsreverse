...
function assign_local_variable(method, index, type){
    if (index <= method.params.length){
        return assign_param_name(method, index, type);
    }
    index -= method.params.length;

    if (method.local_vars[index] === undefined){
        method.local_vars[index] = "var_" + index;
    }
...
function show_decompiled_java_method(method, object, level){
    ...
    for (var i = 0; opcode = method.opcodes[i]; i++){
        ...
        case "istore_0": case "fstore_0": case "dstore_0": case "lstore_0":
        case "istore_1": case "fstore_1": case "dstore_1": case "lstore_1":
        case "istore_2": case "fstore_2": case "dstore_2": case "lstore_2":
        case "istore_3": case "fstore_3": case "dstore_3": case "lstore_3":
            ...
            op.lvalue = assign_local_variable(method, index);
            op.rvalue = stack.pop();
            ops.push(op);
            break;

        case "iload_0": case "fload_0": case "dload_0": case "lload_0":
        case "iload_1": case "fload_1": case "dload_1": case "lload_1":
        case "iload_2": case "fload_2": case "dload_2": case "lload_2":
        case "iload_3": case "fload_3": case "dload_3": case "lload_3":
            ...
            stack.push(
                assign_local_variable(method, index));
            break;
