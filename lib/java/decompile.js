...
function show_decompiled_java_method(method, object, level){
    ...
    var toplevel_ops = [];
    for (var i = 0; i < method.opcodes.length; i++){
        var opcode = method.opcodes[i];
        ...
        // Used multiple times in the switch
        var index,
            type,
            value,
            object_ref,
        ...
        case "ldc":
            value = get_java_constant_comments(object.constantPool,
        ...
        case "aload_3": case "iload_3": case "fload_3": case "dload_3": case "lload_3":
            ...
            nameAndType = assign_local_variable(method, index, type, object);
            stack.push(nameAndType[0]);
            stack_type.push(nameAndType[1]);
            break;


        case "bipush":
        case "sipush":
            for (j = 0; j < opcode.params.length; j++){
                var opcode_param = opcode.params[j];
        ...
        case "putfield":
            value = stack.pop();
            stack_type.pop();
            object_ref = stack.pop();
            stack_type.pop();
            field = object.constantPool[opcode.params[0].value - 1];
            nameAndType = object.constantPool[field.nameAndTypeIndex - 1];
        ...
        case "getfield":
            ...
            field = object.constantPool[opcode.params[0].value - 1];
            nameAndType = object.constantPool[field.nameAndTypeIndex - 1];
            returned_type = descriptor2Type(nameAndType.descriptor);

            if (object_ref !== "this"){
                value = assign_variable_name(method, i, object, returned_type);
                stack.push(value);
        ...
        case "invokespecial":
            ...
            stack.push(object_ref + "(" + param_str + ")");
        ...
        case "invokevirtual":
            ...
            returned_type = get_call_type_and_params(opcode, object)[0];
            ...
            if (returned_type !== "void"){
                ...
                result = assign_variable_name(method, i, object, returned_type);
                op.lvalue_type = returned_type;
                op.lvalue = result;
            }
        ...
        case "new":
            type = object.constantPool[opcode.params[0].value - 1].name;
            params = "";
        ...
        case "getstatic":
            object_ref = object.constantPool[opcode.params[0].value - 1];
            var name = object.constantPool[object_ref.nameAndTypeIndex - 1].name;
            var cls =  object.constantPool[object_ref.classIndex - 1].name;
        ...
        case "ifnonnull":
            ...
            level++;
            block = {border: opcode.params[0].value, op: op, ops: []};
            frame.push(block);
            op.block = block;
            break;
        ...
        case "if_icmple":
            ...
            op.operation = 'if';
            if (op.comparison_right === undefined){
                op.comparison_right = stack.pop();
                stack_type.pop();
            }
            ...
            level++;
            block = {border: opcode.params[0].value + opcode.position - 1,
                         starts_on: opcode.position,
                         op: op,
                         ops: []};
            frame.push(block);
            op.block = block;
        ...
        case "goto":
        case "goto_w":
            var branchoffset = opcode.params[0].value;
            if (branchoffset > 32768){
                branchoffset -= 65536;
            }

            // Probably a loop
            var branchto = opcode.position + branchoffset;
            branchto--;

            if (branchoffset < 0){
                for (var j = frame.length - 1; j >= 0; j--){
                    if (branchto <= frame[j].starts_on){
                        frame[j].op.operation = "while";
                        break;
                    }
                }
            }
        ...
        case "iinc":
            var varname = assign_local_variable(method, opcode.params[0].value,
                                                type, object)[0];
            var increment = opcode.params[1].value;
            op = {};
            if (increment == 1){
...
javaClass.prototype.getSource = function(prefer_bytecode) {
    ...
    // Class flags
    for (i = 0; i < possibleClassFlags.length; i++){
        flag = possibleClassFlags[i];
    ...
    for (i = 0; i < this.fields.length; i++){
        ...
        for (j = 0; j < possibleFieldFlags.length; j++){
            flag = possibleFieldFlags[j];
            if (field.flags[flag]){
    ...
    for (i = 0; i < this.methods.length; i++){
        ...
        for (j = 0; j < possibleMethodFlags.length; j++){
            flag = possibleMethodFlags[j];
